    ...
    stages {
        ...
        stage ('QG2') {
          steps {
            ...
            unstash "repzap"
            sh '''
              err_c=$(grep -iE "\"severity\": \"CRITICAL" trivyout.json | wc -l)
              err_h=$(grep -iE "\"severity\": \"HIGH" trivyout.json | wc -l)
              if [ $err_c -ge 1 ] || [ $err_h -ge 3 ]; then
                echo "Trivy-QualityGate failed"
                exit 1
              fi
              err_c=$(grep -E "\"riskdesc\": \"Critical" zapout.json | wc -l)
              err_h=$(grep -E "\"riskdesc\": \"High" zapout.json | wc -l)
              if [ $err_c -ge 1 ] || [ $err_h -ge 3 ] ; then
        ...
        stage ('TS') {
          steps {
            sh '''
              curl -sSfL https://github.com/toxic-repos/toxic-repos/blob/main/data/json/toxic-repos.json 
              grep -q https://github.com/kserg13/nettu-meet-ks toxic-repos.json && grep -E 'ddos|malware|broken_assembly' toxic-repos.json
              if [ $? -eq 0 ]; then
                  echo "Toxic repos"
                  exit 1
              fi
